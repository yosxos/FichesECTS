...
export const handler = async (event: any = {}): Promise<any> => {
  ...
  try {
   ...
   switch (method) {
    ...
    case 'PUT':
        // Run a UPDATE query
        const queryUp = `UPDATE ${TABLE_NAME} SET name = "${body.name}",prenom = "${body.prenom}" WHERE id = ${body.id}`;
    ...
    case 'POST':
        // Run a INSERT query
        const queryIns = `INSERT INTO ${TABLE_NAME} (name,prenom) VALUES ("${body.name}","${body.prenom}")`;
